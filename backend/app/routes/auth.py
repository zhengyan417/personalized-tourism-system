...
def _find_user_by_username(conn, username: str):
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT user_id, username, password, email FROM users WHERE username=%s LIMIT 1",
            (username,),
        )
        return cur.fetchone()
...
def login():
    ...
    user_data = {
        ...
        "created_at": (
            str(profile_row["created_at"]) if profile_row.get("created_at") else None
        ),
        "updated_at": (
            str(profile_row["updated_at"]) if profile_row.get("updated_at") else None
        ),
    }
...
def me():
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT COUNT(*) as diary_count FROM diaries WHERE user_id=%s", (uid,)
        )
        stats = cur.fetchone()
    ...
    if request.method == "GET":
        ...
        if age is not None:
            try:
                ...
                if age < 0 or age > 150:
                    return (
                        jsonify({"status": "error", "message": "年龄必须在0-150之间"}),
                        400,
                    )
