    ...
    try:
        ...
        for diary in diaries:
            try:
                ...
                print(f"[Community] 解压内容失败: {e}, content type: {type(diary['content'])}")
            ...
            result.append({
                ...
                "nickname": safe_decode(diary["nickname"]) or safe_decode(diary["username"]),
    ...
    try:
        ...
        for comment in comments:
            result.append({
                ...
                "nickname": safe_decode(comment["nickname"]) or safe_decode(comment["username"]),
...
def increment_view(diary_id):
    ...
    try:
        ...
        # 获取更新后的浏览量
        cursor.execute("SELECT view_count FROM diaries WHERE diary_id = %s", (diary_id,))
        result = cursor.fetchone()
