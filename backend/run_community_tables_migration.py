    ...
    try:
        with conn.cursor() as cursor:
            ...
            if 'like_count' not in existing_diary_columns:
                cursor.execute("ALTER TABLE diaries ADD COLUMN like_count INT DEFAULT 0 COMMENT '点赞数'")
                print("  ✓ 添加 like_count 字段")
            else:
                print("  ⊙ like_count 字段已存在")
                
            if 'comment_count' not in existing_diary_columns:
                cursor.execute("ALTER TABLE diaries ADD COLUMN comment_count INT DEFAULT 0 COMMENT '评论数'")
                print("  ✓ 添加 comment_count 字段")
            else:
                print("  ⊙ comment_count 字段已存在")
                
            if 'view_count' not in existing_diary_columns:
                cursor.execute("ALTER TABLE diaries ADD COLUMN view_count INT DEFAULT 0 COMMENT '浏览数'")
                print("  ✓ 添加 view_count 字段")
            else:
                print("  ⊙ view_count 字段已存在")
                
            if 'is_public' not in existing_diary_columns:
                cursor.execute("ALTER TABLE diaries ADD COLUMN is_public TINYINT(1) DEFAULT 1 COMMENT '是否公开（1=公开，0=私密）'")
            ...
            if 'nickname' not in existing_user_columns:
                cursor.execute("ALTER TABLE users ADD COLUMN nickname VARCHAR(50) NULL COMMENT '昵称' AFTER username")
                print("  ✓ 添加 nickname 字段")
            else:
                print("  ⊙ nickname 字段已存在")
                
            if 'avatar_url' not in existing_user_columns:
                cursor.execute("ALTER TABLE users ADD COLUMN avatar_url VARCHAR(500) NULL COMMENT '头像URL' AFTER nickname")
                print("  ✓ 添加 avatar_url 字段")
            else:
                print("  ⊙ avatar_url 字段已存在")
                
            if 'follower_count' not in existing_user_columns:
                cursor.execute("ALTER TABLE users ADD COLUMN follower_count INT DEFAULT 0 COMMENT '粉丝数'")
                print("  ✓ 添加 follower_count 字段")
            else:
                print("  ⊙ follower_count 字段已存在")
                
            if 'following_count' not in existing_user_columns:
                cursor.execute("ALTER TABLE users ADD COLUMN following_count INT DEFAULT 0 COMMENT '关注数'")
