...
- 2025-11-20 路径规划与状态管理增强：
	- 引入 Vuex（vuex@4）并新增 `routePlanning` 模块：state(startPoint/endPoint/waypoints/calculatedRoute/currentMode/loading/error) 与相应 mutations（setStart/setEnd/addWaypoint/clearAll/setCalculatedRoute 等）。
	- Home 路径规划 Tab：新增模式按钮（设起点 / 设终点 / 加途经点 / 清空 / 开始规划）、总距离与步行/骑行时间估算（distanceInfo）。
	- 地图交互：基于 `routePlanningMode` 控制点击地图添加点，自动分配唯一 id；支持起点/终点/途经点不同颜色与序号标签；分段距离继续使用 Haversine 计算。
	- BaseMap 新增 `plannedPath` 渲染：蓝色高亮折线、悬停变色加粗、简易方向箭头（段中点 ➤）。
	- 路径规划请求：新增 `src/api/routePlanning.js` 封装 `planRoute()`；Home 中 `computeRoute()` 统一处理加载态与错误展示（alert + Vuex error）。
	- 稳定性：缩放动画期间延迟 `plannedPath` 重绘避免与 Leaflet zoom 动画冲突；继续使用 `_pendingRouteRender` / `_pendingPlannedRender` 机制。
	- Bug 修复：`routeMarkersForMap` 计算属性防御性重写，解决 `arr.push is not a function` 运行时错误（非数组值被误处理时返回空数组并记录日志）。
	- 传参修正：`:route-mode` 绑定改为依赖 `!!routePlanningMode`（仅规划模式下响应点击）；`:route-markers` 改为从 Vuex 状态组合（起点+途经点+终点）。
	- 结构清理：移除旧的本地 `routeMarkers` data 字段，避免与 Vuex 状态重复。后续将支持推荐结果“一键加入行程”与路线持久化。
	- 下一步计划（未实现）：推荐列表加入“加入行程”操作、后端真实规划算法接入、路线保存与重载、polylineDecorator 优化箭头样式。
	- 新增api文档
